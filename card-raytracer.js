...
function sampler(o, d) {
  ...
  if (m & 1) {
    h = h.prod(0.2);
    return (Math.trunc(Math.ceil(h.x) + Math.ceil(h.y)) & 1 ? v2 : v3).prod(
      b * 0.2 + 0.1
    );
  }
  ...
  return (b > 0 ? new Vector(p, p, p) : v4).sum(sampler(h, r).prod(0.5));
}
//  make trace an generator function which will breaks its execution every 50ms
function* trace(startX, startY, endX, endY) {
  const startTime = Date.now();
  console.log(
    "%s processId: %s, starting rendering with %o",
  ...
  let a = new Vector(0, 0, 1).vprod(g).norm().prod(0.002);
  let b = g.vprod(a).norm().prod(0.002);
  ...
  console.log(
    "%s processId: %s, finished rendering, duration %s ms",
    new Date().toISOString(),
    process.pid,
    Date.now() - startTime
  );
...
// async version to allow the transporter heartbeat with optional callback
async function traceAsync(startX, startY, endX, endY, callback = (v) => v) {
  return new Promise((resolve) => {
    const generator = trace(startX, startY, endX, endY);
    const next = () => {
...
function traceSync(startX, startY, endX, endY, callback = (v) => v) {
  const generator = trace(startX, startY, endX, endY);
  let done = false;
  let value;
  while (!done) {
    const { done: d, value: v } = generator.next();
    done = d;
    value = v;
  }
